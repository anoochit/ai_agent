...
This project demonstrates a Retrieval-Augmented Generation (RAG) pipeline using FAISS for vector storage and Gemini AI for content generation. The pipeline processes a document, splits it into chunks, creates embeddings, and allows users to query the document for relevant information.
...
- **Vector Storage**: Uses FAISS to store and retrieve document embeddings.
...
- Ensure the FAISS index is created before querying. If it doesn't exist, the script will create one automatically.
